...
export function AudioInput({ session, controller, models, modelError, refreshModels, start }: { session: Session; controller: SessionController; models: Schema['ModelsResponseV2'] | null; modelError: string | null; refreshModels: () => void; start: () => void }) {
  ...
  return <section className="input-view"><div className="input-intro"><h1>Từ tiếng nói đến thực thể y tế</h1><p className="muted">Đưa vào một đoạn audio để phiên âm và rà soát.</p></div>
    ...
    <div className="input-notices metadata"><p>Chỉ dùng audio được phép sử dụng.</p><p>Bản demo nghiên cứu, không dùng để chẩn đoán.</p><details><summary>Audio và dữ liệu được xử lý ở đâu?</summary><p>Audio đi qua Cloudflare Tunnel đến web BFF trên máy demo và được xử lý bằng GPU cục bộ trên cùng máy. Audio tạm được xóa sau xử lý hoặc lỗi; kết quả có thể được giữ tối đa 15 phút và mất sớm hơn khi dịch vụ khởi động lại. Phiên trình duyệt chỉ nằm trong bộ nhớ của tab, không tự lưu. Không đưa thông tin bệnh nhân vào demo.</p></details></div>
  </section>;
}
