...
export function EntityPanel({ text, slot, source, model, status, retry, resume, selected, onSelect, prefix, comparison = false }: { text: string; slot: Slot; source: Source; model: Model; status?: Schema['ModelStatus']; retry: () => void; resume: () => void; selected: string | null; onSelect: (id: string) => void; prefix: string; comparison?: boolean }) {
  ...
  const announcement = slot.state === 'succeeded'
    ? `${modelName[model]} hoàn tất, ${count} thực thể.`
    : slot.state === 'failed'
      ? `${modelName[model]} nhận diện thất bại.`
      : slot.state === 'loading'
        ? `${modelName[model]} đang nhận diện.`
        : '';
  return <section className="panel entity-panel" aria-label={`${modelName[model]}: thực thể dự đoán`}><div className="panel-heading"><h2>{comparison ? modelName[model] : `Thực thể dự đoán${count === null ? '' : ` · ${count}`}`}</h2>{comparison && <span className="badge">{modelRole[model]}</span>}</div><p className="sr-only" aria-live="polite">{announcement}</p>
    <div className="entity-tools"><span className="metadata">Nguồn: {source === 'raw' ? 'ASR gốc' : 'Bản rà soát'}{comparison && count !== null ? ` · ${count} thực thể` : ''}</span><label className="sr-only" htmlFor={`${prefix}-filter`}>Lọc thực thể theo loại</label><select id={`${prefix}-filter`} value={filter} onChange={event => setFilter(event.target.value)}><option value="all">Tất cả</option>{kinds.map(label => <option value={label} key={label}>{labels[label]}</option>)}</select></div>
